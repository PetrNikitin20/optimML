...
def build():
    ...
    r = abstract.add_run(
        "Исследуется влияние температурного параметра β на геометрию пакетного градиента и низкоранговую адаптацию reward-модели. Эксперимент выполнен на 2000 реальных парах Anthropic HH-RLHF. В линейной постановке при изменении β от 0,1 до 10 угол градиента достиг 34,71°, а ESS/n уменьшился до 0,7141; максимальная точность парной модели составила 0,5627±0,0072 при β=0,3. Дополнительно предобученный BERT-tiny адаптирован методом LoRA ранга 8: обучались 8321 из 4394241 параметров (0,189%). По трём инициализациям LoRA получила точность 0,5333±0,0200 против 0,5467±0,0058 у замороженного энкодера с обучаемой головой, то есть прирост качества на малой выборке не подтверждён. При этом в пространстве LoRA-параметров β=10 повернул градиент на 30,39°, снизил ESS/n до 0,6199 и насытил 31,25% весов. Результаты подтверждают геометрический механизм в нейросетевой модели и показывают необходимость ранней остановки и нескольких seed."
    )
    ...
    set_font(keywords.add_run("оптимизация предпочтений; геометрия градиента; LoRA; reward-модель; температурное масштабирование; HH-RLHF"), size=9, bold=False, italic=True)
    ...
    results_column_break = doc.add_paragraph()
    results_column_break.paragraph_format.space_after = Pt(0)
    results_column_break.add_run().add_break(WD_BREAK.COLUMN)
    add_table(
        doc,
    ...
    add_body(doc, "К ограничениям относятся один англоязычный датасет, компактный BERT вместо генеративной LLM, три нейросетевых seed, один ранг и отсутствие внешней оценки RewardBench. Дальнейшая работа должна сравнить LoRA, LoRA+ и PiSSA на моделях 1–7 млрд параметров, включить несколько β в обучение, логировать послойные углы и проверять перенос на RewardBench. Для русскоязычного переноса нужен сопоставимый корпус пар и контроль длины ответа.")

    add_heading(doc, "ЗАКЛЮЧЕНИЕ", 1)
    add_body(doc, "Эксперимент на реальных данных HH-RLHF подтвердил три вывода. Во-первых, β является геометрическим гиперпараметром: в линейной модели он повернул градиент до 34,71°. Во-вторых, этот механизм сохранился в пространстве LoRA: при β=10 угол достиг 30,39°, ESS/n уменьшился до 0,6199. В-третьих, обучение 0,189% параметров не гарантировало прироста точности на 1400 парах; ранняя остановка выбирала первую эпоху всех LoRA-запусков. Практически β, функцию потерь и параметризацию адаптера следует выбирать совместно и контролировать через угол градиента, ESS, насыщение весов и внешнюю валидацию.")
